...
const Navbar = () => {
  ...
  const [isServiceMenuDropDownOpen, setServiceMenuDropDownOpen] =
    useState(false);
  const [isIndustryMenuDropDownOpen, setIndustryMenuDropDownOpen] =
    useState(false);
  ...
  return (
    <div
      className={`w-full h-14 bg-black bg-opacity-60 shadow-md lg:h-[80px] fixed z-40 transition-all duration-300 ease-in-out ${
        fix ? "bg-black shadow-md" : ""
      }`}
    >
        ...
        <div className="h-full xl:max-w-[95%] xl:min-w-[90%] w-[90%] m-auto items-center flex justify-between">
          ...
          <div className="text-[18px] hidden xl:flex xl:gap-10 items-center font-gilroy-semi-bold text-white z-10">
            <NavLink
              ...
              className={({ isActive }) =>
                isActive
                  ? "decoration-white underline decoration-2 underline-offset-4"
            ...
            >
              <span
                ...
                // onMouseOver={() => setServiceMenuDropDownOpen(true)}
                className="cursor-pointer"
                // added a onClick functionality to open and close dropdown
                onClick={() => {
                  setServiceMenuDropDownOpen(!isServiceMenuDropDownOpen);
                  handleServices();
                }}
              >
                Services
            ...
            <Link
              ...
              className={`border-white ${
                isServicesPage || isIndustriesPage
                  ? "bg-white text-black"
                  : "bg-transparent text-white"
              } border-2 py-[8px] px-[12px] rounded-full text-[16px] font-gilroy-semi-bold hover:bg-white hover:text-black duration-150 ease-in-out transition-all`}
            >
              Startup Programme
            </Link>
          </div>
          <div className="flex xl:hidden z-50">
            ...
            {!open ? (
              ...
              <img
                onClick={handleOpen}
                src={close}
                className="w-[25px] z-[100] cursor-pointer right-8 top-7 absolute"
              />
            )}
          </div>
